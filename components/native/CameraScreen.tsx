...
// import { CameraType, CameraView } from "expo-camera"; // or expo-camera's Camera


// const { width: screenWidth } = Dimensions.get("window");
// const cameraHeight = (screenWidth * 4) / 3;

// export default function CameraScreen({  setShowCamera,  setImage }: any) {
...
const ASPECT_RATIOS = {
  "4:3": {
    ratio: 4/3,
    getSize: () => ({ width: screenWidth* SCALE_FACTOR, height: screenWidth * (3/4)* SCALE_FACTOR })
  },
  "1:1": {
    ratio: 1,
    getSize: () => ({ width: screenWidth* SCALE_FACTOR, height: screenWidth* SCALE_FACTOR })
  },
  "full": {
  ...
  "16:9": {
    ratio: 16/9,
    getSize: () => ({ width: screenWidth* SCALE_FACTOR, height: screenWidth * (9/16)* SCALE_FACTOR })
  }
};
export default function CameraScreen({ setShowCamera, setImage }:any) {
  const cameraRef = useRef<any>(null);
  const [cameraType, setCameraType] = useState<CameraType>("back");
  const [aspectRatio, setAspectRatio] = useState<keyof typeof ASPECT_RATIOS>("4:3");
  ...
  return (
    <SafeAreaView style={styles.container}>
      <CameraView
        ref={cameraRef}
        style={[styles.camera, { width: cameraDimensions.width, height: cameraDimensions.height }]}
        facing={cameraType}
      />
